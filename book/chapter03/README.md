...
하나의 함수 또는 타입 내에서 지역적으로 채널드을 바인딩하는 select 문을 찾아볼 수 있으며, 시스템 상 두개 이상의 구성 요소가 교차하는 곳에서도 전역으로 바인딩하는 것을 볼 수 있다 

구성 요소들을 합치는 것 외에도 프로그램 상의 중요한 특정 시점에 select 문을 사용해 취소, 시간 초과, 대기 및 기본 값과 같은 개념을 안전하게 채널에 도입할 수 있다 

#### 여러 채널에 읽을 내용이 있을 때는 어떻게 될까 

Go 의 런타임은 case 구문의 집합에 대해 균일한 의사 무작위 선택을 수행한다 

Go 런타임은 select 문의 의도에 대해 알 수 없고 문제 공간을 추론하거나 채널 그룹을 select  구문에 넣은 이유를 추론할 수 없다 

#### 어떠한 채널도 준비되지 않는다면 어떻게 될까

모든 채널이 차단되어 대기하는 경우에 대비해 default 절을 허용한다 

#### 

## GOMAXPROCS 레버

실제로 이 함수는 작업 대기열이라고 불리는 OS 스레드의 수를 제어한다 

```go
runtime.GOMAXPROCS(runtime.NumCPU)
```

1.5 버전 이후에는 호스트 시스템의 논리적인 CPU 수로 자동 설정된다 

이 값을 조정하면 프로그램을 최대한의 속도로 할 수 있지만 추상화나 장기적인 성능 안정성을 희생해야 한다 

성능 조정시 모든 커밋이 이루어질 때마다 사용하는 하드웨어나 Go  버전에 맞추어 조정해야 한다 
